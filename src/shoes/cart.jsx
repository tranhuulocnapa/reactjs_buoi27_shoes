...
export default function Cart(props) {
    const { cartprops, updonwitemcartprops, totalpriceprops } = props
                ...
                <div key={product.id} className="flex items-center justify-between gap-2">
                    ...
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                        <img
                            ...
                            alt={product.name}
                        />
                        <p className="font-medium text-gray-900 dark:text-white break-words whitespace-normal leading-snug min-w-0">
                            {product.name}
                        </p>
                    </div>
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                        <button
                            onClick={() => updonwitemcartprops(product.id, false)}
                            className="w-7 h-7 flex items-center justify-center rounded bg-gray-200 hover:bg-gray-300 font-bold"
                        >
                            -
                        </button>
                        <span className="w-6 text-center">{product.qty}</span>
                        <button
                            onClick={() => updonwitemcartprops(product.id, true)}
                            className="w-7 h-7 flex items-center justify-center rounded bg-sky-500 text-white hover:bg-sky-600 font-bold"
                        >
                            +
                        </button>
                    </div>
                    ...
                    <p className="font-semibold text-gray-900 dark:text-white flex-shrink-0 w-16 text-right">
